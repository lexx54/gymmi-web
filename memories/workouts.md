...
- Assignment forks return an `assignment` window on `GET /workouts/:id`. The builder shows week tags for Mondays overlapping `startDate`–`endDate`; each week owns independent day/exercise/set data through `WorkoutDay.weekStartDate`. A logged client day (`COMPLETED` or `INCOMPLETE`) makes that weekday read-only only in that calendar week. Trainer session list is `GET /workouts/:id/sessions`.
